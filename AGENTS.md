...
- Always: Keep code strictly typed. Do not use `any`, implicit `any`, `@ts-ignore`, or type assertions to silence real type errors. Type assertions are allowed only at explicit trust boundaries such as generated code, API/JSON parsing, DOM/library interop, or literal narrowing with `as const`; otherwise ask first
