...
### Жизненный цикл запуска ассистента

После проверки ассистента backend сразу сохраняет запуск в статусе `pending`. Дальше вызов LLM-провайдера и перевод запуска в `success` или `failed` выполняются с отдельными timeout-контекстами, не завязанными на жизнь HTTP-запроса. Если пользователь закроет вкладку после создания запуска, backend всё равно постарается дозаписать финальный статус, чтобы история не оставалась в `pending`.
